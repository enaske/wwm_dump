Module: hexm.client.ui.windows.rush_tower.rush_tower_const
Type: table
================================================================================

Keys:
  TOWER_QIANSI: number
  get_liupai_level_reward_state: function
  get_base_data_by_tower_type: function
  TOWER_TYPE_STORE: number
  liupai_kongfu_recommend_status: function
  liupai_activity_state_rewarded: number
  get_liupai_activity_limit_info: function
  ACTIVITY_SINGLE_TYPE: number
  LIUPAI_KONGFU_RECOMMEND: number
  TOWER_TYPE_LIUPAI: number
  liupai_activity_red_point_name: string
  _reload_all: boolean
  LIUPAI_KONGFU_FORBID: number
  liupai_activity_state_achieve: number
  TOWER_TYPE_BASIC: number
  liupai_activity_state_unachieve: number
  LIUPAI_KONGFU_UNRECOMMEND: number
  ACTIVITY_ALL_TYPE: number
  TOWER_TYPE_DIANFENG: number