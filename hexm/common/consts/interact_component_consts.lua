Module: hexm.common.consts.interact_component_consts
Type: table
================================================================================

Keys:
  COMP_TYPE_IWORLD_STATIC_ENTITY_PREFIX: string
  COMP_TYPE_INTERACTCOM: number
  WAY_TYPE_COMP: number
  RELATION_TRANSITION_CHANGE: number
  is_ins_static_entity: function
  RELATION_TYPE_TURRET_SHRINK_BONE_TARGET_SHOT: number
  COMP_TYPE_STATIC_ENTITY: number
  CHANGE_WAY_SPEED_GTE: number
  SIMULATE_INTERACT_TYPE_RIDE_SKILL: number
  COMP_TYPE_INS_STATIC_ENTITY: number
  COMP_TYPE_NONE: number
  RELATION_TRANSITION_BUILD: number
  CHANGE_WAY_LEAVE_RADIUS: number
  CHANGE_WAY_EXTERNAL_CONTROL: number
  CHANGE_WAY_ENTER_BATTLE_SKILL_PANEL: number
  PROGRESS_CLIENT_FIRST: number
  FORCE_SAVE_TYPE_SERVER: number
  COMP_TYPE_INS_ENTITY_MATCH_PATTERN: string
  COMP_PRIVATE_TYPE_OWN_LOAD: number
  CHANGE_WAY_ENTER_STATE_AUTO: number
  is_type_interactcom: function
  COMP_TYPE_INS_STATIC_ENTITY_MATCH_PATTERN: string
  is_type_npc: function
  process_need_wait_result_back: function
  COMP_PRIVATE_TYPE_AVATAR: number
  PROGRESS_LOCAL: number
  NPC_INTERACTING_FLAG_BRANCH_SELECT_FINISH: string
  NPC_INTERACTING_FLAG_ACTIVE_INTERACT: string
  COMP_TYPE_LOCAL_NPC: number
  NPC_INTERACTING_FLAG_TALK_OPTIONS: string
  ACTION_COOLDOWN: number
  is_ins_entity: function
  CHANGE_WAY_ENTER_STATE: number
  PROGRESS_NORMAL: number
  _reload_all: boolean
  CHANGE_WAY_ENTER_RADIUS: number
  RELATION_CANCEL: dict
  RELATION_TYPE_TURRET_WATER: number
  FORCE_SAVE_TYPE_CLIENT: number
  RELATION_TYPE_PICKUP: number
  WAY_TYPE_SKILL_PANEL: number
  COMBINE_MODE_ST_DOING: string
  RELATION_TYPE_TURRET: number
  RELATION_EVENT2TRIGGER: dict
  COMP_TYPE_INS_ENTITY: number
  CHANGE_WAY_ACTIVE_INTERACT: number
  COMP_TYPE_NPC: number
  ACTION_READY: number
  ACTIVE_INTERACT_STEP_PRE_RESULT: number
  ACTIVE_INTERACT_STEP_RESULT: number
  (1 more keys)