Module: hexm.common.consts.huiwu_consts
Type: table
================================================================================

Keys:
  ERR_OK: number
  get_show_begin_time: function
  E_BEGIN_BATTLE: number
  E_GET_OTHER_INFO: number
  ERR_NOT_LEADER: number
  E_TEAM_NO_MAP: number
  E_MAP: number
  ERR_NOT_IN_RANGE: number
  E_RESTORE_BARRIER: number
  E_BATTLE_STATE: number
  ERR_CD_MAX: number
  E_GET_BATTLE_INFO: number
  ERR_OTHER_NOT_IN_WORLD: number
  E_ENTER_BATTLE_SAPCE: number
  E_GET_PREPARE: number
  E_GRADE_UP: number
  PREPARE_NOT_PREPARE: number
  E_ENTER_HUIWU_PREPARE: number
  is_in_huiwu_time: function
  HINT_TARGET_NOT_IN_MAIN_SCENE: number
  TEAM_TASK_KEY: string
  HINT_TARGET_NOT_ONLINE: number
  HINT_IN_JINGJICHANG: number
  E_LEAVE_SPACE_ACTIVE: number
  IOS_WIN_NUM: number
  E_REMOVE_BARRIER: number
  E_GET_PLAYER_INFO: number
  E_ROUND_END: number
  IOS_WIN_HUIWU: number
  ERR_LEVEL_LIMIT: number
  E_LEAVE_HUIWU_PREPARE: number
  get_begin_time: function
  ERR_ERR: number
  ERR_QIECUO_TEAM_TYPE: number
  ERR_OTHER_HAS_MAP: number
  attend_commmon_precheck: function
  E_MAX: number
  E_BATTLE_END: number
  ERR_NOT_IN_MAP_INVITE: number
  ERR_IN_COMPETITION: number
  _huiwu_check_time_in_range: function
  PREPARE_NOT_IN: number
  ERR_TIME_LIMIT: number
  E_GET_RESULT: number
  E_ATTEND: number
  E_CHANGE_STATE: number
  ERR_IN_SAME_TEAM: number
  E_ROUND_BATTLE_END: number
  E_PREPARE: number
  ERR_NOT_IN_MAP_REPLY: number
  (1 more keys)